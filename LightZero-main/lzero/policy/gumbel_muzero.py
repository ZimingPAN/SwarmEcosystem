...
class GumbelMuZeroPolicy(MuZeroPolicy):
    ...
    config = dict(
        ...
        # (bool) Whether to rerank root actions by physics-aware energy-rate score.
        use_physics_rate_search=False,
        # (bool) Whether to divide searched completed q by predicted delta_t. If False, use immediate reward / delta_t.
    ...
    def default_model(self) -> Tuple[str, List[str]]:
        ...
        elif self._cfg.model.model_type == "kmc_graph":
            return 'KMCGraphMuZeroModel', ['lzero.model.kmc_graph_muzero_model']
        else:
    ...
    def _compute_physics_rate_scores(
            self,
            model: torch.nn.Module,
            latent_state_root: torch.Tensor,
            legal_actions: List[int],
            completed_values: np.ndarray | None = None,
    ) -> np.ndarray | None:
        if not bool(getattr(self._cfg, 'use_physics_rate_search', False)):
            return None
        if not hasattr(model, 'recurrent_inference') or not hasattr(model, 'latest_time_delta'):
            return None
        if len(legal_actions) == 0:
            return None
        with torch.no_grad():
            latent_state_root = latent_state_root.to(self._cfg.device)
            repeated_latent = latent_state_root.unsqueeze(0).repeat(len(legal_actions), 1)
            action_tensor = torch.tensor(legal_actions, device=self._cfg.device, dtype=torch.long).unsqueeze(-1)
            network_output = model.recurrent_inference(repeated_latent, action_tensor)
            reward_pred = network_output.reward
            if self._cfg.model.categorical_distribution:
                reward_pred = self.reward_inverse_scalar_transform_handle(reward_pred).squeeze(-1)
            else:
                reward_pred = reward_pred.float().squeeze(-1)
            delta_t_pred = model.latest_time_delta.float().clamp(min=1e-8)

            if completed_values is not None and bool(getattr(self._cfg, 'physics_rate_search_use_completed_q', True)):
                base_value = torch.tensor(completed_values[legal_actions], device=self._cfg.device, dtype=torch.float32)
            else:
                base_value = reward_pred.float()
            return (base_value / delta_t_pred).detach().cpu().numpy()
    ...
    def _forward_collect(
        ...
        with torch.no_grad():
            ...
            latent_state_roots_t = latent_state_roots.detach()
            ...
            for i, env_id in enumerate(ready_env_id):
                ...
                if bool(getattr(self._cfg, 'use_physics_rate_search', False)):
                    rate_scores = self._compute_physics_rate_scores(
                        self._collect_model,
                        latent_state_roots_t[i],
                        legal_actions[i],
                        completed_values=np.asarray(roots_completed_value),
                    )
                    if rate_scores is not None:
                        action = int(legal_actions[i][int(np.argmax(rate_scores))])
                    else:
                        action = int(np.argmax([v for v in valid_value]))
                else:
                    action = int(np.argmax([v for v in valid_value]))
