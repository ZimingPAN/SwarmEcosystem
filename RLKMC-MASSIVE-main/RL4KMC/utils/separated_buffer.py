...
class SeparatedReplayBuffer(object):
    def __init__(self, args, act_space):
        ...
        self.num_cu = args.lattice_cu_nums

        # obs_shape = get_shape_from_obs_space(obs_space)
        # share_obs_shape = get_shape_from_obs_space(share_obs_space)
        obs_shape = (self.num_agents, 14,)
        # obs_shape = (self.num_agents, 14,)
        # obs_shape = (self.num_agents, 14,)
        # obs_shape = (self.num_agents, args.topk * 4 + 14,)
        # share_obs_shape = (9 + (args.topk * 4 + 14) * self.num_agents,)
        share_obs_shape = (9 + (14) * self.num_agents,)
        ...
        self.share_obs = np.zeros((self.episode_length + 1, *share_obs_shape), dtype=np.float32)
        # self.obs = np.zeros((self.episode_length + 1, *obs_shape), dtype=np.float32)
        self.obs = np.array([{} for _ in range(self.episode_length + 1)])

        self.rnn_states = np.zeros((self.episode_length + 1,  self.recurrent_N, self.rnn_hidden_size), dtype=np.float32)
        ...
        # vacancy positions at each step
        self.positions = np.zeros((self.episode_length + 1, args.lattice_v_nums, 3), dtype=np.int32)
        ...
        self.actions = np.zeros((self.episode_length,  1), dtype=np.int32)

        self.action_log_probs = np.zeros((self.episode_length, self.num_agents * 8), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, positions,  actions, action_log_probs,
               value_preds, rewards, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step + 1] = np.array(share_obs, copy=True)
        self.obs[self.step + 1] = obs.copy()
        self.positions[self.step + 1] = np.array(positions, copy=True)
        self.actions[self.step] = actions
        self.action_log_probs[self.step] = np.array(action_log_probs, copy=True)
        self.value_preds[self.step] = np.array(value_preds, copy=True)
        self.rewards[self.step] = np.array(rewards, dtype=np.float32, copy=True).reshape(1)
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        for indices in sampler:
            ...
            if self.factor is None:
                ...
                yield share_obs_batch, obs_batch, actions_batch, value_preds_batch, return_batch, old_action_log_probs_batch, adv_targ, factor_batch
